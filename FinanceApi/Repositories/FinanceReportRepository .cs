...
GlLines AS (
    ...
    --------------------------------------------------------
    -- C) Credit Note lines  (Reverse revenue)
    --------------------------------------------------------
    SELECT
...
------------------------------------------------------------
-- 5) CONVERT FINAL BALANCE → DEBIT / CREDIT FOR TB
------------------------------------------------------------
FinalTB AS (
    SELECT
        HeadId,
        HeadCode,
        HeadName,
        -- balance same formula as your GL
        CASE 
...
FROM FinalTB
WHERE Balance <> 0          -- hide pure zero rows
ORDER BY HeadCode;
";

            return await Connection.QueryAsync<TrialBalanceDTO>(sql, new
            {
                dto.FromDate,   // not used yet, but kept for future
                dto.ToDate,
                dto.CompanyId
            });
        }
...
;WITH GlLines AS (
    ...
    --------------------------------------------------------
    -- B) Sales Invoice lines  (Revenue side)
    --------------------------------------------------------
    SELECT
        si.InvoiceDate        AS TransDate,
        'SI'                  AS SourceType,
...
";
            ...
            return await Connection.QueryAsync<TrialBalanceDetailDTO>(
                ...
                new
                {
                    ...
                    dto.FromDate,
                    dto.ToDate,
                    dto.CompanyId  // not used in SQL yet, but keeps signature same
                });
        }
    }
